...
public class GameBoard {
    ...
    public GamePiece checkCollisionAt(int x, int y){
        for (int i = 0; i < getNumGamePieces(); i++) {
            for (int x1 = 0; x1 < getGamePieces()[i].getWidth(); x1++) {
                for (int y1 = 0; y1 < getGamePieces()[i].getHeight(); y1++) {
                    if (getGamePieces()[i].getTopLeftX()+x1 == x && getGamePieces()[i].getTopLeftY()+y1 == y)
                        return getGamePieces()[i];
                }
            }
        }
    ...
    public void checkCompletion(GamePiece gp) {
        if (gp.topLeftX == GameBoard.WIDTH)
            completed = true;
    }
}