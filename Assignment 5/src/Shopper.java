public class Shopper {
    ...
    public String printReceipt() {
        String receipt = "";
        byte descriptionLength = 30, priceLength = 15;
        for (int i = 0; i < numItems; i++) {
            if (cart[i] instanceof GroceryBag) {
                GroceryItem[] itemsInBag = ((GroceryBag) cart[i]).getItems();
                for (int j = 0; j < ((GroceryBag) cart[i]).getNumItems(); j++)
                    receipt += String.format("%-" + descriptionLength + "s%," + priceLength + ".2f%1s", itemsInBag[j].getDescription(), itemsInBag[j].getPrice(), "\n");
            } else {
                receipt += String.format("%-" + descriptionLength + "s%," + priceLength + ".2f%1s", cart[i].getDescription(), cart[i].getPrice(), "\n");
            }
        }
        for (int i = 1; i <= (descriptionLength + priceLength); i++)
            receipt += "-";
        receipt += String.format("\n%-" + descriptionLength + "S%," + priceLength + ".2f", "total", this.computeTotalCost());
        return receipt;
    }
}